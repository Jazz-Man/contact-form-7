...
use JazzMan\ContactForm7\Integration\WPCF7_Service_OAuth2;
...
class WPCF7_ConstantContact extends WPCF7_Service_OAuth2 {
    ...
    public const authorization_endpoint
        = 'https://authz.constantcontact.com/oauth2/default/v1/authorize';

    public const token_endpoint
    ...
    private function __construct() {
        ...
        add_action('wpcf7_admin_init', [$this, 'auth_redirect']);
    }
    ...
    public function auth_redirect(): void {
        ...
        && current_user_can('wpcf7_manage_integration')) {
    ...
    public function load($action = ''): void {
        if ('auth_redirect' == $action) {
            ...
            if ($code && $state
            && wpcf7_verify_nonce($state, 'wpcf7_constant_contact_authorize')) {
    ...
    public function update_contact_lists($selection = []) {
        ...
        foreach ((array) $selection as $key => $ids_or_names) {
            foreach ($contact_lists as $list_id => $list) {
                ...
                || in_array($list['name'], (array) $ids_or_names, true)) {
    ...
    protected function menu_page_url($args = ''): string {
        ...
        $url = add_query_arg(['service' => self::service_name], $url);
