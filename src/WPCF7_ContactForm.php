...
use WP_Query;
use JazzMan\ContactForm7\Swv\WPCF7_SWV_SchemaHolder;
...
use function wpcf7_autop;
use function wpcf7_autop_or_not;
use function wpcf7_create_nonce;
use function wpcf7_deprecated_function;
use function wpcf7_enctype_value;
use function wpcf7_format_atts;
use function wpcf7_get_request_uri;
use function wpcf7_get_validation_error_reference;
use function wpcf7_is_valid_locale;
use function wpcf7_mail_replace_tags;
use function wpcf7_messages;
use function wpcf7_switch_locale;

use const WP_DEBUG;
use const WPCF7_VERIFY_NONCE;
use const WPCF7_VERSION;
...
class WPCF7_ContactForm {
	...
	private function __construct( $post = null ) {
		...
		if ( $post
		     && self::post_type == get_post_type( $post ) ) {
			$this->id     = $post->ID;
	...
	public function __get( string $name ) {
		$message = __( '<code>%1$s</code> property of a <code>JazzMan\ContactForm7\WPCF7_ContactForm</code> object is <strong>no longer accessible</strong>. Use <code>%2$s</code> method instead.',
			'contact-form-7' );
	 ...
	 *
	 * @param  array|string  $args  Optional. Arguments to be passed to WP_Query.
	 *
	 * @return WPCF7_ContactForm[] array of JazzMan\ContactForm7\WPCF7_ContactForm objects
	 */
	 ...
	 *
	 * @param  array|string  $args  Optional. Contact form options.
	 *
	 * @return WPCF7_ContactForm a new contact form object
	 */
	public static function get_template( $args = '' ): ?self {
	...
	/**
	 * Returns an instance of JazzMan\ContactForm7\WPCF7_ContactForm.
	 *
	 * @param  mixed  $post
	 *
	 * @return WPCF7_ContactForm a new contact form object
	 */
	public static function get_instance( $post ): ?self {
		$post = get_post( $post );
	 ...
	 *
	 * @return null|array|string Property value. Null if property does not exist.
	 */
	 ...
	 *
	 * @return null|string Attribute value. Null if the attribute does not exist.
	 */
	 ...
	 *
	 * @param  array|string  $args  Optional. Form options.
	 *
	...
	public function form_do_shortcode() {
		wpcf7_deprecated_function(
			__METHOD__,
			'4.6',
			'JazzMan\ContactForm7\WPCF7_ContactForm::replace_all_form_tags'
		);
	 ...
	 *
	 * @param  null|array|string  $cond  Optional. Filters. Default null.
	 *
	 * @return \JazzMan\ContactForm7\WPCF7_FormTag[] form-tags matching the given filter conditions
	 */
	...
	public function form_scan_shortcode( $cond = null ) {
		wpcf7_deprecated_function(
			__METHOD__,
			'4.6',
			'JazzMan\ContactForm7\WPCF7_ContactForm::scan_form_tags'
		);
	 ...
	 *
	 * @param  array|string  $args  Optional. Search options.
	 *
	 ...
	 */
	public function suggest_mail_tags( string $template_name = 'mail' ): void {
		$mail = wp_parse_args(
	 ...
	 *
	 * @param  array|string  $args  Optional. Submission options. Default empty.
	 *
	 ...
	 * @param  null|mixed  $post
	 */
	private function construct_properties( $post = null ): void {
		$builtin_properties = [
			'form'                => '',
			'mail'                => [],
			'mail_2'              => [],
