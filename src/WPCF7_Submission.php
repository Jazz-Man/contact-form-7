...
use function wp_kses_no_null;
use function wp_parse_args;
use function wp_unslash;
use function wpcf7_array_flatten;
use function wpcf7_form_tag_supports;
use function wpcf7_get_request_uri;
use function wpcf7_is_name;
use function wpcf7_rmdir_p;
use function wpcf7_sanitize_unit_tag;
use function wpcf7_switch_locale;
use function wpcf7_unship_uploaded_file;
use function wpcf7_verify_nonce;

use const REST_REQUEST;
use const WPCF7_USE_PIPE;
...
class WPCF7_Submission {
	 ...
	 *
	 * @param  null|mixed  $contact_form
	 * @param  mixed  $args
	 */
	public static function get_instance( $contact_form = null, $args = '' ): ?self {
	...
	public function get_result(): array {
		$result = array_merge( $this->result_props, [
	 ...
	 *
	 * @param  array|object|string  $args  value to add to result properties
	 *
	 ...
	 *
	 * @return null|string the meta information of the given name if it exists,
	 *                     null otherwise
	 */
	 ...
	 *
	 * @return null|array|string the user input of the field, or array of all
	 ...
	 * Adds a spam log.
	 *
	 * @see https://contactform7.com/2019/05/31/why-is-this-message-marked-spam/
	 *
	 * @param  mixed  $args
	 */
	public function add_spam_log( $args = '' ): void {
		$args = wp_parse_args( $args, [
			'agent'  => '',
			'reason' => '',
		] );
	 ...
	 *
	 * @param  array|string  $file_path  a file path or an array of file paths
	...
	private function setup_posted_data() {
		$posted_data = array_filter( $_POST, fn( $key ) => '_' !== substr( $key, 0, 1 ), ARRAY_FILTER_USE_KEY );
		...
		foreach ( (array) $tags as $tag ) {
			...
			if ( wpcf7_form_tag_supports( $type, 'selectable-values' ) ) {
				...
				if ( $tag->has_option( 'free_text' )
				     && isset( $posted_data[ $name . '_free_text' ] ) ) {
					...
					[ $last_val, $tied_item ] = array_map(
						function ( $item ) {
							return wpcf7_canonicalize( $item, [
								'strto' => 'as-is',
							] );
						},
						[ $last_val, $tied_item ]
	...
	private function create_posted_data_hash( $tick = '' ): string {
		...
		return wp_hash(
			...
			'JazzMan\ContactForm7\WPCF7_Submission'
		);
	}
	...
	private function verify_nonce() {
		if ( ! $this->contact_form->nonce_is_active() || ! is_user_logged_in() ) {
			return true;
		}
	 ...
	 *
	 * @param  string  $name  field name
	 * @param  array|string  $file_path  file path or array of file paths
	 */
	 ...
	 */
	private function remove_uploaded_files(): void {
		foreach ( (array) $this->uploaded_files as $file_path ) {
			...
			foreach ( $paths as $path ) {
				...
				if ( ( $dir = dirname( $path ) ) && false !== ( $files = scandir( $dir ) )
				     && ! array_diff( $files, [ '.', '..' ] ) ) {
