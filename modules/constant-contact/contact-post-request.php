...
class WPCF7_ConstantContact_ContactPostRequest {
    ...
    private $street_addresses = [];

    private $list_memberships = [];

    public function __construct() {
    }

    public function build(WPCF7_Submission $submission): void {
        ...
        && $your_name = $submission->get_posted_string('your-name')) {
        ...
        $contact_lists = $service_option['contact_lists'] ?? [];
    ...
    public function set_create_source($create_source) {
        if (!in_array($create_source, ['Contact', 'Account'], true)) {
            return false;
        }
    ...
    public function set_birthday($month, $day) {
        ...
        return [$this->birthday_month, $this->birthday_day];
    }
    ...
    public function add_phone_number(string $phone_number, string $kind = 'home') {
        ...
        || 2 <= count($this->phone_numbers)
        || !in_array($kind, ['home', 'work', 'other'], true)) {
            return false;
        }
